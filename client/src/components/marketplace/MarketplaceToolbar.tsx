...
const selCls = "bg-charcoal/80 border border-white/15 rounded-lg px-3 py-1.5 text-xs text-chalk focus:outline-none focus:border-gold/50 transition-colors cursor-pointer shrink-0";
...
export function MarketplaceToolbar(p: Props) {
  return (
    <div className="sticky top-14 z-30 bg-obsidian/95 backdrop-blur-md border-b border-white/8 px-4 py-3 space-y-3">
      ...
      <div className="flex w-full rounded-xl border border-white/10 overflow-hidden h-10">
        <button
      ...
      {/* Row 2 — flex-wrap: overflows go to next line on small screens */}
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          ...
          Filters
          {p.activeFilterCount > 0 && (
            <span className="bg-gold text-obsidian w-4 h-4 rounded-full text-[10px] font-bold flex items-center justify-center shrink-0">
              {p.activeFilterCount}
            </span>
          )}
        </button>

        {/* Tier chips — wrap together if no space */}
        {TIERS.map((t) => (
          <button
            key={t}
            onClick={() => p.setTier(t)}
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <ArrowUpDown className="w-3.5 h-3.5 text-chalk-faint" />
        ...
        {p.selectedCount > 0 && (
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-xs text-gold font-semibold whitespace-nowrap">{p.selectedCount} selected</span>
            <button onClick={p.onClearSelection} className="p-1 text-chalk-faint hover:text-chalk transition-colors rounded">
              <X className="w-3.5 h-3.5" />
            </button>
            <button onClick={p.onAddSelectedToCart} className="purple-pill px-3 py-1.5 text-xs font-bold flex items-center gap-1.5 whitespace-nowrap">
              <ShoppingCart className="w-3.5 h-3.5" /> Add {p.selectedCount}
            </button>
          </div>
        )}
